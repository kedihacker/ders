...
func mamaloop(wg *sync.WaitGroup, totalpairs *int64) {
	...
	for x := 0; x < 8; x++ {
		...
		conn, httpres, err := websocket.DefaultDialer.Dial("wss://yenicericopybackend.herokuapp.com/a/"+randuuid.String(), nil)
	...
	atomic.AddInt64(totalpairs, int64(localpairs))
	wg.Done()
}
